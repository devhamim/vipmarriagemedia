@foreach ($users as $user)
    <tr>
        ...
        <td>
            @if ($user->profile_img)
                <img src="{{ asset('storage/users/pp/' . $user->profile_img) }}" alt=""
                    class="img-fluid border border-danger" style="max-width: 80px; ">
            @else
                <img src="{{ asset('img/vip-user.png') }}" alt="" class="img-fluid border border-danger"
        ...
        <td>
            @if ($user->uploadedPP())
                ...
                <img class="img-fluid border border-danger" style="max-width: 80px; "
        ...
        <td>
            ...
            <strong>Profile Created At:</strong> {{ date('d-M-Y', strtotime($user->created_at)) }} <small>(
                {{ Carbon\Carbon::parse($user->created_at)->diffForHumans() }}) </small> <br>
    ...
    Loged By: {{$user->firstLog() ? $user->firstLog()->logAddedBy->email : ""}} --}}
            ...
            <div class="btn-group btn-group-xs pull-right w-100">
                ...
                <ul class="dropdown-menu pl-2" role="menu">
                    ...
                    @can('user-edit')
                        <li>
                            <a
                                href="{{ route('admin.makeUserActive', $user) }}">{{ $user->active ? 'Deactive' : 'Active' }}</a>
                        </li>
                    @endcan
                    @can('user-log')
                        <li>
        ...
        {{ $user->active ? 'Active' : 'Deactivated' }}
            ...
            <div class="btn-group btn-group-xs pull-right w-100">
                {{-- <button type="button" data-url="" class="btn btn-primary">Edit</button> --}}
                <a class="btn btn-success" href="{{ route('admin.logs', $user) }}">Logs({{ $user->countLogs() }})</a>

            </div>
            {{-- <div class="mt-2">
                @if ($user->editedby_id)
                    @if ($user->editedby_id and $user->editedby_id != $user->id)
                        <p class="text-dark w-100"><span
                                class="badge badge-dark">{{ $user->active ? 'Active' : 'Deactivated' }}
                                By:</span>{{ $user->editedBy->name }}</p>
                    @endif
                @endif
                @if ($user->countLogs())
                    @php

                        $userId = $user->lastLogsName()->addedby_id;

                    @endphp
                    @if ()
                        
                    @endif
                    <p class="text-dark w-100"><span class="badge badge-dark">Last Logs
                            By:</span>{{ $user->firstLog()->logAddedBy->name }}</p>
                @endif
            </div> --}}
            <div class="mt-2">
                @if($user->editedby_id && $user->editedby_id != $user->id && $user->editedBy)
                    <p class="text-dark w-100">
                        <span class="badge badge-dark">{{ $user->active ? 'Active' : 'Deactivated' }} By:</span>
                        {{ $user->editedBy->name }}
                    </p>
                @endif
                ...
                @if($user->countLogs())
                    @php
                        $lastLog = $user->firstLog();
                    @endphp
            
                    @if($lastLog && $lastLog->logAddedBy)
                        <p class="text-dark w-100">
                            <span class="badge badge-dark">Last Logs By:</span>
                            {{ $lastLog->logAddedBy->name }}
                        </p>
                    @endif
                @endif
            </div>
